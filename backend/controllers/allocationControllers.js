...
export const allocateTeacher = async (req, res, next) => {
  ...
  if (req.user.role !== 'admin') {
    return res.status(403).json({ success: false, message: 'Only admin users can allocate teachers' });
  }
  ...
  try {
    ...
    const teachers = await Teacher.find({ subjects: subjectId }).sort({ teachingExperience: -1, teachingHours: 1 });
    ...
    res.status(201).json({ success: true, message: 'Allocation created successfully', data: allocation });
  } catch (error) {
    next(error);
  }
};
...
export const updateAllocation = async (req, res, next) => {
  // Check if the user is admin
  if (req.user.role !== 'admin') {
    return res.status(403).json({ success: false, message: 'Only admin users can update allocations' });
  }
  ...
  try {
    const updatedAllocation = await Allocation.findByIdAndUpdate(id, { timing }, { new: true });
    if (!updatedAllocation) {
      return res.status(404).json({ success: false, message: 'Allocation not found' });
    }

    res.status(200).json({ success: true, message: 'Allocation updated successfully', data: updatedAllocation });
  } catch (error) {
    next(error);
  }
};
...
export const deleteAllocation = async (req, res, next) => {
  // Check if the user is admin
  if (req.user.role !== 'admin') {
    return res.status(403).json({ success: false, message: 'Only admin users can delete allocations' });
  }
